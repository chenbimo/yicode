...
async function fnRequire(filePath, defaultValue, fromType = 'core') {
    const rootDir = path.resolve(fromType === 'core' ? sysConfig.yiapiDir : sysConfig.appDir, 'yiapi.js');
    const pathDir = fnFileProtocolPath(rootDir);
    const require = createRequire(pathDir);
...
async function fnGetTableData(allTableName) {
    try {
        ...
        for (let i = 0; i < tableFileAll.length; i++) {
            ...
            const tableDataItem = await fnRequire(filePath, {}, 'user');
            // 设置表名称、描述
